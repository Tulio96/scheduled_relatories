    ...
    try:
        with engine.connect() as connection:
            with connection.begin(): # Inicia uma transação
                ...
                dados_devedor = df_resultados["CodProcesso"].astype('Int64').replace({pd.NA: None})

                dados_devedor = df_resultados["CodProcesso"].dropna(how="all")

                dados_query = list(set(dados_devedor))
                ...
                SELECT 
                        COUNT(CallId)                               AS [Acionamentos_Olos],
                ...
                df_resultados["CodProcesso"] = df_resultados["CodProcesso"].astype(str)
