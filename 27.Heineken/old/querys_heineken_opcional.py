...
import traceback
import re
from ler_excel_opcional import clean_excel_string
...
def executar_querys(dados_lista,caminho_arquivo,sheet_name): # Recebe a LISTA de tuplas (cnpj, titulo, parcela)
    ...
    """
    caminho_salvamento = r"/mnt/Rede/MIS/B2B - Global/27 - Heineken/Avaliacao_PD_opcional.xlsx"
    engine = get_engine()
        ...
        with engine.connect() as connection:
            with connection.begin(): # Inicia uma transação
                ...
                SELECT
                    ...
                    SUM(
                        IIF(ccs.CodScoreTipo IN ('1','2','4'), 1, 0)
                       )                                            AS [Alo],
                    SUM(
                ...
                FROM
                    ...
                    INNER JOIN ClienteContatoScore ccs ON ccs.CodProcessoHistoricoTipo = ph.CodProcessoHistoricoTipo
                ...
                df_resultados = pd.merge(df_heineken, df_query, on=["Codigo", "OPCIONAL", "Parcela"], how="left")
